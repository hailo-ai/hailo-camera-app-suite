...
/// TODO: Remove this condision when MSW-6216 is done
extern std::string g_output_file_path;
bool is_jpegenc_in_path() {
    return g_output_file_path.find("jpegenc") != std::string::npos;
}

DspImagePropertiesPtr dspbuffer;
tl::expected<std::shared_ptr<osd::Overlay>, media_library_return> custom_expected;
...
void OSD_scenario(uint counter, MediaLibraryEncoderPtr encoder)
{
    ...
    switch (counter)
    {
        ...
        case 1:
            ...
            custom_expected = encoder->get_blender()->get_overlay("custom_argb1");
            existing_custom_overlay = std::static_pointer_cast<osd::CustomOverlay>(custom_expected.value());
            dspbuffer = existing_custom_overlay->get_buffer();
       ...
       */
        case 10:
            text_overlay.label = "Chaos App_edit";
...
void privacy_mask_scenario(uint counter, MediaLibraryFrontendPtr frontend)
{
...
void encoder_scenario(int counter, MediaLibraryEncoderPtr encoder, bool& g_encoder_is_running)
{
    EncoderType encoding_format = encoder->get_type();
    if (encoding_format == EncoderType::Hailo) {
        /// TODO: Remove this condision when MSW-6216 is done
        if ((counter % 100 == 0) && (!is_jpegenc_in_path())) {
            if (counter % 200 == 0)
            {
        ...
        /*
        if ((counter % 75 == 0) && (!is_jpegenc_in_path())) {
            if (counter % 150 == 0)
            {
    ...
    */
    ...
    if ((counter % 60 == 0) || (counter % 60 == 5)) {
        if ((counter % 60 == 0) && (g_encoder_is_running))
        {
            std::cout << "stopping encoder" << std::endl;
            encoder->stop();
            g_encoder_is_running = false;
        }
        else if ((counter % 60 == 5) && (!g_encoder_is_running))
        {
...
void vision_scenario(uint counter, MediaLibraryFrontendPtr frontend)
{
    ...
    /// TODO: MSW-6136 - When setting grayscale to enable dynamically the stream crashes with errors
   /* if (counter % 29 == 0)
    {
