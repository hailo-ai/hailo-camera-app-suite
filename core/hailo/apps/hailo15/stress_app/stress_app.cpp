...
ParsedOptions parseArguments(int argc, char* argv[]) {
    ...
    options.add_options()
        ...
        ("ai-pipeline", "will the ai pipeline be added to the overall pipeline", cxxopts::value<bool>()->default_value("true"))
    ...
    parsedOptions.ai_pipeline_enabled = result["ai-pipeline"].as<bool>();
...
void create_pipeline(std::shared_ptr<MediaLibrary> m_media_lib, bool leaky_ai_queues)
{
    ...
    std::shared_ptr<HailortAsyncStage> detection_stage = std::make_shared<HailortAsyncStage>(DETECTION_AI_STAGE, YOLO_HEF_FILE, 5, 50 ,"device0", 10, 8, std::chrono::milliseconds(100), false);
    //
    ...
    std::shared_ptr<HailortAsyncStage> landmarks_stage = std::make_shared<HailortAsyncStage>(LANDMARKS_AI_STAGE, LANDMARKS_HEF_FILE, 20, 101 ,"device0", 1, 1, std::chrono::milliseconds(100), false);
