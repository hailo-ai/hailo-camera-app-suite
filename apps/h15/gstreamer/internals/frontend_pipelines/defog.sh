...
function init_variables() {
    ...
    readonly DEFAULT_DEFOG_CONFIG_FILE_PATH="$RESOURCES_DIR/configs/defog_config.json"

    input_source=$DEFAULT_VIDEO_SOURCE
    defog_config_file_path="$DEFAULT_DEFOG_CONFIG_FILE_PATH"
...
function print_usage() {
    echo "Defog streaming pipeline usage:"
    echo ""
    echo "Options:"
    ...
    echo "  --defog-config-file-path  Set the defog config file path (default ${DEFAULT_DEFOG_CONFIG_FILE_PATH})"
    exit 0
}
...
function parse_args() {
    while test $# -gt 0; do
        if [ "$1" = "--print-gst-launch" ]; then
            ...
            input_source="$2"
            shift
        elif [ "$1" = "--defog-config-file-path" ]; then
            defog_config_file_path="$2"
            shift
        else
...
PIPELINE="${debug_stats_export} gst-launch-1.0 \
    ...
    hailodefog config-file-path=$defog_config_file_path name=defog ! \
