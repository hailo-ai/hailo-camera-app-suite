...
IspResource::IspResource(std::shared_ptr<AiResource> ai_res, std::shared_ptr<ConfigResource> config_res, std::shared_ptr<OsdResource> osd_res) : Resource(), m_baseline_stream_params(0, 0, 0, 0, 0), m_baseline_wdr_params(0), m_baseline_backlight_params(0, 0)
{
    ...
    this->m_v4l2->v4l2_ext_ctrl_set<bool>(V4l2_CTRL_AE_HCG, false);
    ...
    osd_res->subscribe_callback([this](ResourceStateChangeNotification notification)
                                {
        if (notification.resource_type != RESOURCE_OSD)
        {
            return;
        }
        auto state = std::static_pointer_cast<OsdResource::OsdResourceState>(notification.resource_state);
        for (OsdResource::OsdResourceConfig<osd::AutoFocusOverlay> &autofocus_overlay : state->autofocus_overlays)
        {
            if (std::find(state->overlays_to_delete.begin(), state->overlays_to_delete.end(), autofocus_overlay.get_overlay().id) != state->overlays_to_delete.end() ||
                !autofocus_overlay.get_enabled())
            {
                this->m_v4l2->v4l2_ext_ctrl_set<bool>(V4l2_CTRL_AF_ENABLE, false);
            }
            else if (autofocus_overlay.get_enabled())
            {
                this->m_v4l2->v4l2_ext_ctrl_set<bool>(V4l2_CTRL_AF_ENABLE, true);
            }
            else
            {
                WEBSERVER_LOG_ERROR("ISP: autofocus overlay enabled state is not set");
            }
        }
    });
...
void IspResource::on_ai_state_change(std::shared_ptr<AiResource::AiResourceState> state)
{
    ...
    this->m_v4l2->v4l2_ext_ctrl_set<bool>(V4l2_CTRL_AE_HCG, denoise_enabled);
